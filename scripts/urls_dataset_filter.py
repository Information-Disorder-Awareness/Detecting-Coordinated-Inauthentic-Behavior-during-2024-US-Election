...
def process_urls(platform: Platform) -> None:
    ...
    if os.path.exists(f"./datasets/{platform}/filtered_urls.csv"):
    ...
    filtered_df.to_csv(f"./datasets/{platform}/filtered_urls.csv", index=False)
    print(f"Correctly filtered {len(filtered_df)} urls in dataset/{platform}/dataset.csv")
