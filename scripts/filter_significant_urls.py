...
def get_relevant_urls(url_stats: pd.DataFrame) -> pd.Series:
    ...
    mean = counts.mean()
    std = counts.std()
    filtered = url_stats[counts > (mean + std)]
...
def filter_relevant_urls(platform: Platform) -> None:    
    ...
    if os.path.exists(f"./datasets/{platform}/filtered_relevant_urls.csv"):
        print("Dataset already filtered for relevant urls")
        return

    print("Starting dataset filtering for relevant urls...")
    df = pd.read_csv(f'./datasets/{platform}/filtered_urls.csv')
    filtered_df = filter_texts_with_urls(df, relevant_urls)

    filtered_df.to_csv(f'./datasets/{platform}/filtered_relevant_urls.csv', index=False)
